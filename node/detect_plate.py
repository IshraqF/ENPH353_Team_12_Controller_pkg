...
class plate_detection():
    ...
    def publish_plate(self, cv_image):
    ...
    def display_plate(self, img, cv_image):
        print("display plate")
    ...
    def cutout_letters(self, img, cv_image):
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        ...
        for imagez in letters:
            char = self.display_plate(imagez, cv_image)
    ...
    def cutout_plate_number(self, img, cv_image):
    ...
    def detect_plate(self, image, cv_image):
        ...
        # print("^^warped")
            ...
            self.cutout_plate_number(warped, cv_image)
            self.cutout_letters(warped, cv_image)
    
    def detect_car(self, cv_image):
        image = cv_image
        hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
        ...
        if len(contours) > 2:
            ...
            if cv2.contourArea(sortedContours[0]) > 29000 and cv2.contourArea(sortedContours[0]) < 52000:
                ...
                self.detect_plate(crop, cv_image)
